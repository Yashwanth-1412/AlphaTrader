...
int main(int argc, char** argv) {
    ...
    if (ne_pid == 0) {
        ...
        execlp(nanoexchange_bin.c_str(), nanoexchange_bin.c_str(), gateway_iface.c_str(), gport.c_str(), inc_mcast.c_str(), iport.c_str(), snapshot_tcp.c_str(), mcast_iface.c_str(), "1", "2", "3",
               (char*)nullptr);
    ...
    MarketDataConsumer                 consumer(&md_queue, &logger, mcast_iface, "127.0.0.1", snapshot_tcp_port, inc_mcast, inc_port, inc_port + 1);
    consumer.start(-1);
    for (int i = 0; i < 20 && !market_data_synchronized.load(std::memory_order_acquire); ++i) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    check("TCP snapshot synchronized market data", market_data_synchronized.load(std::memory_order_acquire));
    ...
    auto send_enter = [&](const std::string& token, char side, uint32_t shares, uint32_t ticker_id, uint32_t price_scaled, bool read_resp = false) -> std::string {
    ...
    auto send_replace = [&](const std::string& old_token, const std::string& new_token, uint32_t shares, uint32_t price_scaled) -> bool {
        int fd = tcp_connect("127.0.0.1", gateway_port);
        if (fd < 0)
            return false;
    ...
    auto s1 = drain_queue();
    check("received " + std::to_string(s1.size()) + " AddOrder updates (expect >= 6)", s1.size() >= 6);
    int a_count = 0;
    for (auto& m : s1)
        if (m.type == UpdateType::ADD)
            ++a_count;
    check("all are type=A (AddOrder)", a_count == (int)s1.size());

    if (g_verbose)
        for (auto& m : s1)
            std::cout << "  seq=" << m.seq_num << " type=" << (char)m.type << " side=" << (char)m.side << " qty=" << m.qty << " price=" << m.price << "\n";
    ...
    for (auto& m : s2)
        if (m.type == UpdateType::ADD)
            found_add_cancel = true;
    check("received AddOrder for canceled order", found_add_cancel);
    if (g_verbose)
        for (auto& m : s2)
            std::cout << "  seq=" << m.seq_num << " type=" << (char)m.type << " side=" << (char)m.side << " qty=" << m.qty << "\n";
    ...
    if (g_verbose)
        for (auto& m : s3)
            std::cout << "  seq=" << m.seq_num << " type=" << (char)m.type << " side=" << (char)m.side << " ref=" << m.order_ref << " new_ref=" << m.new_ref << "\n";
    ...
    int  exec_count = 0;
    for (auto& m : s4) {
        if (m.type == UpdateType::EXECUTE) {
            found_exec = true;
            ++exec_count;
        }
    }
    check("received Execute (E) updates", found_exec);
    check("got " + std::to_string(exec_count) + " execution messages (expect >= 1)", exec_count >= 1);
    if (g_verbose)
        for (auto& m : s4)
            std::cout << "  seq=" << m.seq_num << " type=" << (char)m.type << " side=" << (char)m.side << " qty=" << m.qty << "\n";
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    auto s5 = drain_queue();
    check("received " + std::to_string(s5.size()) + " updates for 4 tickers (expect >= 4)", s5.size() >= 4);
    if (g_verbose)
        for (auto& m : s5)
    ...
    // ════════════════════════════════════════════════════════════
    // SCENARIO 8 — Replay protocol against the real server
    // ════════════════════════════════════════════════════════════
    std::cout << "─── Scenario 8: Replay protocol (real server) ───\n";
    {
        // Ensure fresh increments so we know the server's last published seq.
        send_enter("SC8_A", 'B', 100, 0, 1000000);
        send_enter("SC8_B", 'S', 100, 0, 2000000);
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        auto   s8       = drain_queue();
        SeqNum last_seq = 0;
        for (const auto& m : s8)
            last_seq = std::max(last_seq, m.seq_num);
        check("server has published increments (last_seq=" + std::to_string(last_seq) + ")", last_seq >= 4);

        auto raw_replay = [&](SeqNum client_seq, char& status, std::vector<MarketUpdate>& out) -> bool {
            const int fd = tcp_connect("127.0.0.1", snapshot_tcp_port);
            if (fd < 0)
                return false;
            char req[1 + sizeof(SeqNum)];
            req[0]            = 'R';
            const uint64_t be = htobe64(client_seq);
            std::memcpy(req + 1, &be, sizeof(be));
            ...
            while (offset < stream.size()) {
                MarketUpdate u;
                const auto   wire = std::span<const char>(stream.data() + offset, stream.size() - offset);
                if (!dec.decode(wire, u))
                    break;
                offset += sizeof(SeqNum) + dec.lastDecodedSize();
                out.push_back(u);
            }
            return true;
        };

        // 1) Mid-history replay: client_seq = last_seq - 3 → exactly 3 frames
        std::vector<MarketUpdate> replayed;
        char                      status = 0;
        bool                      ok     = raw_replay(last_seq - 3, status, replayed);
        check("mid-history replay granted (status 'R')", ok && status == 'R');
        check("replayed " + std::to_string(replayed.size()) + " frames (expect 3)", replayed.size() == 3);
        bool contiguous = replayed.size() == 3;
        for (size_t i = 0; i < replayed.size(); ++i) {
            contiguous = contiguous && (replayed[i].seq_num == last_seq - 2 + i);
        }
        check("replayed frames contiguous (seq " + std::to_string(last_seq - 2) + ".." + std::to_string(last_seq) + ")", contiguous);

        // 2) Already current: client_seq == last_seq → 'C', no frames
        replayed.clear();
        ok = raw_replay(last_seq, status, replayed);
        check("already-current returns 'C' with no frames", ok && status == 'C' && replayed.empty());

        // 3) Full history: client_seq = 0 → replays everything from seq 1
        replayed.clear();
        ok = raw_replay(0, status, replayed);
        check("full-history replay granted (status 'R')", ok && status == 'R');
        check("full replay starts at seq 1 (" + std::to_string(replayed.size()) + " frames)", !replayed.empty() && replayed.front().seq_num == 1);
        bool full_contiguous = true;
        for (size_t i = 0; i < replayed.size(); ++i) {
            full_contiguous = full_contiguous && (replayed[i].seq_num == i + 1);
        }
        check("full replay contiguous", full_contiguous);
    }
    ...
    // ── Results ─────────────────────────────────────────────────
    std::cout << "\n═══ Results: " << (g_total_failed == 0 ? "ALL PASS" : "SOME FAILED") << "  (" << g_total_passed << " passed, " << g_total_failed << " failed) ═══\n";
