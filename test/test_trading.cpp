...
int main() {
    ...
    using Engine = FeatureEngine<MktPriceFeature, SpreadFeature, OrderBookImbalanceFeature, AggTradeRatioFeature>;
    Engine features;
    ...
    check("features invalid without two-sided BBO",
          features.get<MktPriceFeature>().value() == Price_INVALID && features.get<SpreadFeature>().value() == Price_INVALID && features.get<OrderBookImbalanceFeature>().value() == Ratio_INVALID);
    ...
    check("imbalance remains scaled integer", features.get<OrderBookImbalanceFeature>().value() == 333'333);
    ...
    check("trade ratio infers aggressor from resting side", features.get<AggTradeRatioFeature>().value() == 600'000);
    ...
    check("partial close PnL", pnl.get(1)->net_qty == 60 && pnl.get(1)->realized_pnl == 800'000 && pnl.get(1)->unrealized_pnl == 600'000);
    check("flip from long to short", pnl.onExecution(1, Side::SELL, 1'030'000, 100));
    check("flip resets entry and recalculates mark",
          pnl.get(1)->net_qty == -40 && pnl.get(1)->avg_entry_price == 1'030'000 && pnl.get(1)->realized_pnl == 2'600'000 && pnl.get(1)->unrealized_pnl == 800'000);
    check("close position", pnl.onExecution(1, Side::BUY, 1'020'000, 40));
    check("flat position clears entry and unrealized",
          pnl.get(1)->net_qty == 0 && pnl.get(1)->avg_entry_price == Price_INVALID && pnl.get(1)->unrealized_pnl == 0 && pnl.get(1)->realized_pnl == 3'000'000);
    ...
    check("reject unknown risk ticker", risk.canSubmit(unknown_ticker, *pnl.get(1)) == RiskResult::UNKNOWN_TICKER);
    ...
    check("reject projected position breach", risk.canSubmit(too_much, *pnl.get(1)) == RiskResult::POSITION_LIMIT_EXCEEDED);
    check("reject per-order quantity breach", risk.canSubmit(too_large, *pnl.get(1)) == RiskResult::ORDER_QTY_EXCEEDED);

    const OrderResponse partial_fill{101, 0, Side::BUY, 0, 0, 30, 1'000'000, 0, OrderResponse::EXECUTED};
    check("process partial execution", risk.onOrderResponse(partial_fill, pnl));
    check("execution transfers reserved exposure to confirmed position", pnl.get(1)->net_qty == 30 && risk.pendingExposure(1) == 30);
    check("still reject with partial reservation", risk.canSubmit(too_much, *pnl.get(1)) == RiskResult::POSITION_LIMIT_EXCEEDED);

    const OrderResponse canceled{101, 0, Side::BUY, 0, 0, 0, 0, 30, OrderResponse::CANCELED};
    ...
    const OrderResponse rejected{102, 0, Side::BUY, 0, 0, 0, 0, 0, OrderResponse::REJECTED};
    check("rejection releases full reservation", risk.onOrderResponse(rejected, pnl) && risk.pendingCount() == 0);
    risk.disableTrading();
    ...
    using RunnerFeatures = FeatureEngine<MktPriceFeature, SpreadFeature, OrderBookImbalanceFeature, AggTradeRatioFeature>;
    using TradeEngineT   = TradeEngine<RunnerFeatures, LiquidityTaker, 1, 1, 4>;
    TradeEngineT engine(&market_updates, &order_requests, &order_responses, &logger, LiquidityTaker({10, 300'000}), 1, nullptr, 64);
    check("register strategy ticker", engine.registerTicker(0, 1, {20, 10, 20'000'000}));

    market_updates.push(add(1, Side::BUY, 1'000'000, 200, 10));
    market_updates.push(add(1, Side::SELL, 1'001'000, 100, 11));
    const StrategyRunResult decision = engine.runOnce();
    OrderRequest            sent{};
    check("engine consumes market queue", decision.market_updates == 2);
    check("engine emits IOC after signal and risk check", decision.orders_submitted == 1 && order_requests.pop(sent) && sent.client_order_id == 1 && sent.ticker_id == 1 && sent.side == Side::BUY &&
                                                              sent.price == 1'001'000 && sent.qty == 10 && sent.ioc);
    check("engine reserves submitted order", engine.risk(0).pendingExposure(1) == 10);

    order_responses.push({1, 0, Side::BUY, 0, 0, 10, 1'000'500, 0, OrderResponse::EXECUTED});
    const StrategyRunResult execution = engine.runOnce();
    check("engine consumes execution response", execution.responses == 1 && engine.pnl(0).get(1)->net_qty == 10 && engine.pnl(0).get(1)->avg_entry_price == 1'000'500);
    check("execution releases IOC reservation", engine.risk(0).pendingCount() == 0);
