...
auto fromWire32(uint32_t value) noexcept -> uint32_t { return swap32(value); }
auto toWire64(uint64_t value) noexcept -> uint64_t { return swap64(value); }
...
auto testServer(int port, std::atomic<bool>& ready, std::atomic<bool>& protocol_ok) -> void {
    ...
    const bool read_ok = readAll(client_fd, &enter, sizeof(enter)) &&
                         readAll(client_fd, &cancel, sizeof(cancel)) &&
                         readAll(client_fd, &replace, sizeof(replace));

    const bool input_ok = read_ok &&
        enter.type == enums::MsgType::ENTER_ORDER &&
        parseToken(enter.order_token) == 101 &&
        enter.buy_sell_indicator == 'B' &&
        fromWire32(enter.shares) == 25 &&
        tickerFromStock(enter.stock) == 1 &&
        fromWire32(enter.price) == 1234567 &&
        fromWire32(enter.time_in_force) == 99998 &&
        cancel.type == enums::MsgType::CANCEL_ORDER &&
        parseToken(cancel.order_token) == 101 &&
        fromWire32(cancel.shares) == 10 &&
        replace.type == enums::MsgType::REPLACE_ORDER &&
        parseToken(replace.existing_order_token) == 101 &&
        parseToken(replace.replacement_order_token) == 102 &&
        fromWire32(replace.shares) == 15 &&
        fromWire32(replace.price) == 1234568 &&
        fromWire32(replace.time_in_force) == 0;
    ...
    if (input_ok) {
        ...
        std::memcpy(replaced.replacement_order_token, replace.replacement_order_token,
...
int main(int argc, char* argv[]) {
    ...
    if (!live_exchange) {
        requests.push(OrderRequest{OrderRequestType::REPLACE, 101, 102, TickerId_INVALID,
                                    Side::BUY, 1234568, 15, true});
    }
    ...
    if (!live_exchange) check("NanoExchange inbound OUCH frames", protocol_ok.load(std::memory_order_acquire));
    check(live_exchange ? "received two NanoExchange responses" : "received three OUCH responses",
          count == expected);
    check("accepted response", count > 0 && received[0].client_order_id == 101 &&
           received[0].market_order_id != 0 && received[0].price == 1234567 &&
          received[0].qty == 25 && received[0].status == OrderResponse::ACCEPTED &&
          (live_exchange || received[0].market_order_id == 9001));
    check("canceled response", count > 1 && received[1].client_order_id == 101 &&
          received[1].status == OrderResponse::CANCELED &&
           (live_exchange || received[1].canceled_qty == 15));
    if (!live_exchange) {
        check("replaced response", count > 2 && received[2].client_order_id == 102 &&
               received[2].market_order_id == 9002 && received[2].price == 1234568 &&
              received[2].qty == 15 && received[2].status == OrderResponse::ACCEPTED);
    }
