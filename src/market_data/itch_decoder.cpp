...
auto ItchDecoder::decode(std::span<const char> wire, MarketUpdate& out) noexcept -> bool {
    ...
    // Resolve the payload size first so truncation (recoverable: wait for more
    // bytes) is separable from an unknown type (unrecoverable at this offset).
    size_t payload_size = 0;
    switch (msg_type) {
    case static_cast<char>(enums::MsgType::SYSTEM_EVENT):
        payload_size = sizeof(SystemEvent);
        break;
    case static_cast<char>(enums::MsgType::ADD_ORDER):
        payload_size = sizeof(AddOrder);
        break;
    case static_cast<char>(enums::MsgType::ADD_ORDER_MPID):
        payload_size = sizeof(AddOrderMPID);
        break;
    case static_cast<char>(enums::MsgType::ORDER_EXECUTED):
        payload_size = sizeof(OrderExecuted);
        break;
    case static_cast<char>(enums::MsgType::ORDER_CANCEL):
        payload_size = sizeof(OrderCancel);
        break;
    case static_cast<char>(enums::MsgType::ORDER_DELETE):
        payload_size = sizeof(OrderDelete);
        break;
    case static_cast<char>(enums::MsgType::ORDER_REPLACE):
        payload_size = sizeof(OrderReplace);
        break;
    default:
        return false; // unknown type — caller must resynchronise
    }

    if (wire.size() < sizeof(SeqNum) + payload_size) {
        need_more_data_ = true;
        return false;
    }
    ...
    const char* payload = wire.data() + sizeof(SeqNum);
    last_decoded_size_  = payload_size;

    switch (msg_type) {
    case static_cast<char>(enums::MsgType::SYSTEM_EVENT):
        return decodeSystemEvent(reinterpret_cast<const SystemEvent*>(payload), out);
    case static_cast<char>(enums::MsgType::ADD_ORDER):
        return decodeAddOrder(reinterpret_cast<const AddOrder*>(payload), out);
    case static_cast<char>(enums::MsgType::ADD_ORDER_MPID):
        return decodeAddOrderMPID(reinterpret_cast<const AddOrderMPID*>(payload), out);
    case static_cast<char>(enums::MsgType::ORDER_EXECUTED):
        return decodeOrderExecuted(reinterpret_cast<const OrderExecuted*>(payload), out);
    case static_cast<char>(enums::MsgType::ORDER_CANCEL):
        return decodeOrderCancel(reinterpret_cast<const OrderCancel*>(payload), out);
    case static_cast<char>(enums::MsgType::ORDER_DELETE):
        return decodeOrderDelete(reinterpret_cast<const OrderDelete*>(payload), out);
    case static_cast<char>(enums::MsgType::ORDER_REPLACE):
        return decodeOrderReplace(reinterpret_cast<const OrderReplace*>(payload), out);
    default:
        return false;
    }
}
...
auto ItchDecoder::decodeAddOrder(const AddOrder* msg, MarketUpdate& out) noexcept -> bool {
    ...
    if (slot.size() <= out.order_ref)
        slot.resize(out.order_ref + 1);
...
auto ItchDecoder::decodeOrderReplace(const OrderReplace* msg, MarketUpdate& out) noexcept -> bool {
    ...
    auto& slot = it->second;
    if (slot.size() <= out.new_ref)
        slot.resize(out.new_ref + 1);
