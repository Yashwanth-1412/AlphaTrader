...
auto MarketDataConsumer::startReplaySync(SeqNum client_seq) noexcept -> void {
    market_data_synchronized.store(false, std::memory_order_release);
    in_recovery_.store(true, std::memory_order_release);
    ...
    addr.sin_port = htons(snapshot_tcp_port_);
    ASSERT(inet_pton(AF_INET, snapshot_tcp_ip_.c_str(), &addr.sin_addr) == 1,
           "MarketDataConsumer::startReplaySync() invalid TCP snapshot address");
    const int result = connect(snapshot_tcp_fd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr));
    ...
    ASSERT(send(snapshot_tcp_fd_, request, sizeof(request), MSG_NOSIGNAL) == static_cast<ssize_t>(sizeof(request)),
           "MarketDataConsumer::startReplaySync() TCP request failed");
}
...
auto MarketDataConsumer::processReplay() noexcept -> void {
    if (!replay_status_read_) {
        if (snapshot_tcp_buffer_.empty()) return;
    ...
    while (offset < snapshot_tcp_buffer_.size()) {
        ...
        const auto wire = std::span<const char>(snapshot_tcp_buffer_.data() + offset,
                                                snapshot_tcp_buffer_.size() - offset);
        if (!decoder_.decode(wire, update)) break;
...
auto MarketDataConsumer::onSnapshotTcpEof() noexcept -> void {
    ...
    if (recovery_mode_ == RecoveryMode::Replay && replay_status_read_) {
        ...
        close(snapshot_tcp_fd_);
        snapshot_tcp_fd_ = -1;
        startReplaySync(replay_last_applied_);
        return;
    }
