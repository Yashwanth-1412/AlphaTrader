...
int main() {
    ...
    for (size_t i = 0; i < s3.size(); ++i) {
        if (s3[i].seq_num != static_cast<SeqNum>(3 + i)) contiguous = false;
    }
    check("replayed updates are contiguous (seq 3..7)", contiguous);
    int replay_orders = 0, queued_orders = 0;
    for (auto& m : s3) {
        if (m.order_ref >= 103 && m.order_ref <= 106) ++replay_orders;
        if (m.order_ref == 102) ++queued_orders;
    }
    ...
    check("replay rebuilds the order book", book.trackedOrders() == 5 &&
          book.getBBO().bid_price == 1500000 && book.getBBO().bid_qty == 300);
    ...
    for (auto& m : s5) {
        ...
        if (m.order_ref == 300) ++dropped_orders;    // from queued incremental
    }
    ...
    for (const auto& update : s5) book2.applyUpdate(update);
    check("snapshot rebuilds the order book", book2.trackedOrders() == 1 &&
          book2.getBBO().bid_price == 1200000 && book2.getBBO().bid_qty == 500);
    check("market data is synchronized after snapshot",
          market_data_synchronized.load(std::memory_order_acquire));
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    auto s6 = drain();
    check("resumed: new AddOrder (seq=10001) received",
          s6.size() == 1 && s6[0].seq_num == 10001);
