...
MarketOrderBook::MarketOrderBook(TickerId ticker_id, Logger*, size_t pool_size) noexcept
...
auto MarketOrderBook::eraseOrder(OrderId order_ref) noexcept -> bool {
    ...
    if (UNLIKELY(it == order_index_.end()))
        return false;
    ...
    if (location.level->head == nullptr) {
        if (location.side == Side::BUY)
            bids_.erase(location.level->price);
        else
            asks_.erase(location.level->price);
    }
...
auto MarketOrderBook::reduceOrder(const MarketUpdate& update) noexcept -> bool {
    ...
    if (UNLIKELY(it == order_index_.end()))
        return false;
    if (update.qty >= it->second.order->qty)
        return eraseOrder(update.order_ref);
...
auto MarketOrderBook::replaceOrder(const MarketUpdate& update) noexcept -> bool {
    const auto it = order_index_.find(update.order_ref);
    if (UNLIKELY(it == order_index_.end() || order_index_.contains(update.new_ref)))
        return false;
    ...
    if (old_level->head == nullptr) {
        if (old_side == Side::BUY)
            bids_.erase(old_level->price);
        else
            asks_.erase(old_level->price);
    }
...
auto MarketOrderBook::applyUpdate(const MarketUpdate& update) noexcept -> void {
    ...
    if (update.type == UpdateType::SNAPSHOT_CLEAR) {
        if (update.ticker_id == ticker_id_)
            onSnapshotClear();
        return;
    }
    ...
    case UpdateType::ADD_MPID: {
        if (UNLIKELY(order_index_.contains(update.order_ref)))
            break;
