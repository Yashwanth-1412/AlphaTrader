...
auto toWire32(uint32_t value) noexcept -> uint32_t { return swap32(value); }
auto toWire64(uint64_t value) noexcept -> uint64_t { return swap64(value); }

} // namespace

OrderGateway::OrderGateway(quantlink::SPSCQueue<OrderRequest>* outgoing,
...
auto OrderGateway::start() -> void {
    if (run_.exchange(true, std::memory_order_acq_rel)) return;

    if (tcp_socket_.connect(ip_, iface_, port_, false) < 0) {
        run_.store(false, std::memory_order_release);
        logger_->log("OrderGateway: failed to connect to %:%\n", ip_.c_str(), port_);
        return;
    }
...
auto OrderGateway::run() noexcept -> void {
    while (run_.load(std::memory_order_acquire)) {
        ...
        while (outgoing_requests_->pop(req)) {
            switch (req.type) {
                case OrderRequestType::NEW:     sendEnter(req); break;
                case OrderRequestType::CANCEL:  sendCancel(req); break;
                case OrderRequestType::REPLACE: sendReplace(req); break;
            }
        }
    }
}
...
auto OrderGateway::messageSize(char type) noexcept -> size_t {
    switch (static_cast<enums::MsgType>(type)) {
        case enums::MsgType::ORDER_ACCEPTED:  return sizeof(OrderAccepted);
        case enums::MsgType::ORDER_EXECUTED:  return sizeof(OrderExecuted);
        case enums::MsgType::ORDER_CANCELED:  return sizeof(OrderCanceled);
        case enums::MsgType::REPLACE_ORDER:   return sizeof(OrderReplaced);
        case enums::MsgType::CANCEL_REJECTED: return sizeof(CancelRejected);
        default:                               return 0;
    }
}
...
auto OrderGateway::recvCallback(quantlink::TCPSocket* socket, quantlink::Nanos) noexcept -> void {
    ...
    while (offset < available) {
        ...
        if (available - offset < size) break;
