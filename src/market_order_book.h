...
public:
    ...
    [[nodiscard]] auto getBBO() const noexcept -> const BBO& { return bbo_; }
    [[nodiscard]] auto getBestBid() const noexcept -> const PriceLevel* { return best_bid_; }
    [[nodiscard]] auto getBestAsk() const noexcept -> const PriceLevel* { return best_ask_; }
    [[nodiscard]] auto empty() const noexcept -> bool { return bids_.empty() && asks_.empty(); }
    [[nodiscard]] auto getTickerId() const noexcept -> TickerId { return ticker_id_; }
    [[nodiscard]] auto trackedOrders() const noexcept -> size_t { return order_index_.size(); }
    [[nodiscard]] auto bidLevels() const noexcept -> size_t { return bids_.size(); }
    [[nodiscard]] auto askLevels() const noexcept -> size_t { return asks_.size(); }
    [[nodiscard]] auto isInSnapshot() const noexcept -> bool { return in_snapshot_; }
