...
// Per-strategy risk — rejects orders before they reach the exchange.
//
// RiskResult        ACCEPTED, TRADING_DISABLED, INVALID_REQUEST, UNKNOWN_TICKER,
//                   DUPLICATE_ORDER, PENDING_CAPACITY_EXCEEDED, ORDER_QTY_EXCEEDED,
//                   ORDER_NOTIONAL_EXCEEDED, POSITION_LIMIT_EXCEEDED
// RiskLimits        {max_abs_position, max_order_qty, max_order_notional}
//
// Public API:
//   bool registerTicker(ticker_id, limits)
//   RiskResult canSubmit(request, position)     check position/qty/notional/capacity
//   bool onSubmitted(request)                   reserve exposure, record pending
//   bool onReplaced(old_id, new_id)             re-key pending under the new id
//   bool onOrderResponse(response, pnl)         release reservation, update pnl on fills
//   void disableTrading() / tradingEnabled()    kill switch
//   pendingExposure(ticker_id) / pendingCount() outstanding reservations
...
class RiskManager {
  public:
    bool registerTicker(TickerId ticker_id, RiskLimits limits) noexcept {
        if (limits.max_abs_position < 0 || limits.max_order_notional < 0)
            return false;
        if (findTicker(ticker_id))
            return false;
        if (ticker_count_ == MaxTickers)
            return false;
    ...
    RiskResult canSubmit(const OrderRequest& request, const Position& position) const noexcept {
        if (!trading_enabled_)
            return RiskResult::TRADING_DISABLED;
        if (request.type != OrderRequestType::NEW || request.client_order_id == 0 || request.ticker_id == TickerId_INVALID || request.price == Price_INVALID || request.price < 0 || request.qty == 0) {
            return RiskResult::INVALID_REQUEST;
        }

        const TickerLimits* ticker = findTicker(request.ticker_id);
        if (!ticker)
            return RiskResult::UNKNOWN_TICKER;
        if (findPending(request.client_order_id))
            return RiskResult::DUPLICATE_ORDER;
        if (pending_count_ == MaxPendingOrders)
            return RiskResult::PENDING_CAPACITY_EXCEEDED;
        if (request.qty > ticker->limits.max_order_qty)
            return RiskResult::ORDER_QTY_EXCEEDED;
        ...
        const int64_t  requested_qty = request.side == Side::BUY ? static_cast<int64_t>(request.qty) : -static_cast<int64_t>(request.qty);
        const __int128 projected     = static_cast<__int128>(position.net_qty) + pendingExposure(request.ticker_id) + requested_qty;
    ...
    bool onSubmitted(const OrderRequest& request) noexcept {
        if (findPending(request.client_order_id) || pending_count_ == MaxPendingOrders)
            return false;
        pending_[pending_count_++] = {request.client_order_id, request.ticker_id, request.side, request.qty};
        return true;
    }

    // Re-key a pending reservation after a replace: the exchange now reports
    // fills/cancels under the replacement client order id.
    bool onReplaced(OrderId old_client_order_id, OrderId new_client_order_id) noexcept {
        if (new_client_order_id == 0 || findPending(new_client_order_id))
            return false;
        const size_t index = findPendingIndex(old_client_order_id);
        if (index == MaxPendingOrders)
            return false;
        pending_[index].client_order_id = new_client_order_id;
        return true;
    }
    ...
    bool onOrderResponse(const OrderResponse& response, PnLTracker<PnLMaxTickers>& pnl) noexcept {
        ...
        case OrderResponse::EXECUTED:
            if (response.executed_qty == 0 || response.executed_qty > pending.reserved_qty || !pnl.onExecution(pending.ticker_id, pending.side, response.execution_price, response.executed_qty)) {
                return false;
            }
            pending.reserved_qty -= response.executed_qty;
            if (pending.reserved_qty == 0)
                erasePending(index);
            return true;
    ...
    int64_t pendingExposure(TickerId ticker_id) const noexcept {
        ...
        for (size_t i = 0; i < pending_count_; ++i) {
            if (pending_[i].ticker_id != ticker_id)
                continue;
            exposure += pending_[i].side == Side::BUY ? static_cast<int64_t>(pending_[i].reserved_qty) : -static_cast<int64_t>(pending_[i].reserved_qty);
        }
        return exposure;
    }
  ...
  private:
    ...
    void erasePending(size_t index) noexcept { pending_[index] = pending_[--pending_count_]; }
